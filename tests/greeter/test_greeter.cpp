...
TEST_F(GreeterTest, TestSimpleGreetFormat) {
    ...
    EXPECT_THAT(my_greeter.simple_greet(), testing::MatchesRegex("Hello from the Greeter class!*"));
}
...
class GreeterDayTest : public testing::TestWithParam<std::tuple<int, std::string>> {};
...
TEST_P(GreeterDayTest, GreetsCorrectlyForDay) {
    ...
    auto mock_provider {std::make_unique<MockDateProvider>()};
    EXPECT_CALL(*mock_provider, getDayOfWeek()).WillOnce(testing::Return(day_of_week));
...
class GreeterEnthusiasmTest : public testing::TestWithParam<std::tuple<float, std::string>> {
   protected:
    Greeter my_greeter;
};
...
class GreeterNameTest : public testing::TestWithParam<std::tuple<std::string, std::string>> {};
